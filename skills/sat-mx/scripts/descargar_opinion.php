...
/**
 * descargar_opinion.php — Descarga Opinión de Cumplimiento del SAT México.
 *
...
try {
    ...
    $scraper = new Scraper($client, $captchaSolver, $rfc, $ciec);
    ...
    $bytes = file_put_contents($output, (string) $content);

    if ($bytes === false || $bytes < 1000) {
        fail("La descarga falló o el PDF está incompleto (bytes={$bytes}).");
    }

    fwrite(STDERR, "✓ Opinión de Cumplimiento descargada: {$output} ({$bytes} bytes)\n");
    echo $output . PHP_EOL;
    exit(0);
} catch (Throwable $e) {
    fail($e->getMessage(), 2);
